 ...
 * @Date: 2021-04-02
 */
public class ProcessOracle implements ProcessDatabase {

  private static final Logger logger = LoggerFactory.getLogger(ProcessOracle.class);
  ...
  public void process(JdbcConnect jdbcConnect, Properties properties) throws Exception{
    ...
    sqlstr = "insert into "+chktbl+"(dt) values(to_date('"+dtChar+"','yyyy-mm-dd hh24:mi:ss'))";
    jdbcConnect.update(conn1, sqlstr);

    sqlstr = "select to_char(dt,'yyyy-mm-dd hh24:mi:ss') as dt from "+chktbl+" order by dt desc";
